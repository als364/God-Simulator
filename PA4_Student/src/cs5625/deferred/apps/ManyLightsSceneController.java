...
public class ManyLightsSceneController extends SceneController
{
	...
	public void initializeScene()
	{
		try
		{
			...
			for (int i = 0; i < 30; ++i)
			{
				...
				Color3f lightColor = new Color3f(Color.getHSBColor((float)Math.random(), 0.5f + 0.5f * (float)Math.random(), 1.0f));
				light.setColor(lightColor);
				
				/* Pick a random position for the light. */ 
				light.setPosition(new Point3f(
						((float)Math.random() - 0.5f) * 8.0f, 
						((float)Math.random() - 0.5f) * 8.0f, 
						((float)Math.random() - 0.5f) * 8.0f));
	...
	{
		/* Preview the requested gbuffer texture, if any. */
